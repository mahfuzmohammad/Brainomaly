...
if [[ $# -lt 2 ]]; then
  echo "Correct usage: bash test_inductive.sh <dataset_folder_name> <model_iteration>"
  exit
fi
...
python main.py --mode testAUCInductive --dataset MedicalData --image_size 192 \
                ...
                --neptune_id ${NEPTUNE_ID} --neptune_key Test_Inductive
